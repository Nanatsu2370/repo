...
tl_admin_users = ['Rohi89','FriendXfriendss','FriendXfriendss2'] #Poner aqui los user con acceso permanente
godlist = ['Yama_Tsukami','ElJoker63','Gabino001'] #Poner aqui los admin
...
async def onmessage(bot:TelegramClient,ev: NewMessage.Event,loop,ret=False):
    ...
    if username not in tl_admin_users:
        ...
        await bot.send_message('ElJoker63','🛑 El usuario @'+username+' ha intentado acceder al bot🛑')
        return
    ...
    if '/start' in text:
        ...
        message = await bot.send_file(ev.chat.id,'thumb.png', caption=f'{reply}', buttons=[Button.url('📯ElJoker63','https://t.me/ElJoker63')])
        #await bot.send_message(ev.chat.id,reply,parse_mode='html')
        pass
    if '/add' in text and username in godlist:
        message = await bot.send_message(ev.chat.id,'⏳Añadiendo a @'+username+' a la lista de acceso')
        usernameadd = text.split(' ')[1]
        tl_admin_users.append(usernameadd)
        await bot.edit_message(ev.chat,message,text=tl_admin_users)
        #await bot.edit_message(ev.chat,message,text='💢Error De Enlace🔗')
        print(tl_admin_users)

    if '/ban' in text and username in godlist:
        message = await bot.send_message(ev.chat.id,'⏳Removiendo a @'+username+' de la lista de acceso')
    ...
    if '/up' in text:
        ...
        try:
            ...
            urls = []
            await bot.edit_message(ev.chat,message,text=f'🔗Guardando enlaces...')
            ...
            await bot.send_file(ev.chat,txtsendname,
                                ...
                                thumb='thumb.png',
                                buttons=[Button.url('📯ElJoker63','https://t.me/ElJoker63')])
...
def init():
    try:
        ...
        bot.send_message('ElJoker63','Bot is Started!')
